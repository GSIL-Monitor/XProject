    ...
    $.get("registry_center", {}, function(data) {
        ...
        for (var i = 0;i < data.length;i++) {
            ...
            if (true === data[i].activated) {
            	baseTd += "<td style='color:green;'>已连接</td>";
                $("#activated-reg-center").text(data[i].name);
                operationTd = "<td><button disabled operation='connect' class='btn' regName='" + data[i].name + "'>已连</button>";
            } else {
            	baseTd += "<td style='color:red;'>未连接</td>";
        ...
        $.post("registry_center", {name: name, zkAddressList: zkAddressList, namespace: namespace, digest: digest}, function(data) {
            ...
            if (data) {
                ...
                if (name != $("#activated-reg-center").text()) {
                	...
                	baseTd += "<td style='color:green;'>已连接</td>";
                    operationTd = "<td><button disabled operation='connect' class='btn' regName='" + name + "'>已连</button></td>";
                }
