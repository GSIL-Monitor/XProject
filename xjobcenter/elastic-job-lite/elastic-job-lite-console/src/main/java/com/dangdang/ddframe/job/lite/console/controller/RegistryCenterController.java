...
public class RegistryCenterController {
    ...
    public Collection<RegistryCenterConfiguration> load(final HttpSession session) {
    	Boolean isAdmin = (Boolean) session.getAttribute(CasConstants.IS_ADMIN);
    	if (isAdmin) {
    		...
    		String username = (String) session.getAttribute(CasConstants.USERNAME);
    		return regCenterService.loadByName(username);
    	}
